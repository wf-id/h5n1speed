...
#' @param mode the transmission modality one of "density" or "frequency"
#' @param reference_size the reference herd size at which density-mode realized R0 equals r0_in
...
run_intervention_ode <- function(
  ...
  simple_sir <- function(time_in, state, params) {
    with(as.list(c(state, params)), {
      ...
      N <- sum(state)
      denom <- if (mode == "density") reference_size else N
      dS <- -beta * S * (I + A) / denom
      dI <- beta * S * (I + A) / denom * (1 - p_asymptomatic) - gamma_use * I
      dA <- beta * S * (I + A) / denom * p_asymptomatic - gamma_use_asymptomatic * A
